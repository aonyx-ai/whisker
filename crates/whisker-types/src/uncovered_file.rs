...
/// Error raised when no decoration provider claimed a file
///
/// Carried through [`anyhow::Error`] so a caller can recognize an analysis
/// failure by downcast and say something more useful about it than the
/// generic error path would. The alternative — collapsing "I could not
/// analyze this" into the same shape as "I analyzed this and found
/// nothing" — is the failure this type exists to prevent.
///
/// # Examples
///
/// ```
...
///      rust: the file is inside the workspace at /ws but no crate in that workspace reaches it",
/// );
/// ```
///
...
impl UncoveredFile {
    ...
    ///
    /// An empty `gaps` means no providers were consulted at all, which
    /// renders as its own message rather than as an empty list.
    ...
    ///
    /// Empty when no decoration providers were configured at all.
...
mod tests {
    ...
    fn display_with_two_gaps_lists_both() {
        ...
        assert_eq!(
            lines[1],
            "  rust: the file changed after the workspace was loaded"
        );
        assert_eq!(
            lines[2],
            "  other: the file is outside the workspace loaded at /ws"
        );
    ...
    fn gaps_returns_every_provider_consulted() {
        ...
        assert_eq!(gaps[1].1, CoverageGap::OutsideWorkspace { root: root() });
    }
