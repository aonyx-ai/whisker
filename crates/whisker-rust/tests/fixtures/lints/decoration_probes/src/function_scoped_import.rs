...
mod tests {
    ...
    /// Pins the answer for a qualifier the provider resolved to nothing
    ///
    /// An unresolved qualifier is not proof of an enum, so the exemption
    /// is withheld and the diagnostic stands.
    #[test]
    fn import_of_an_unresolved_qualifier_is_flagged() {
        let diagnostics = run_with("fn f() { use Mystery::thing; }", ImportSource::Unresolved);
    ...
    /// Pins the answer for a qualifier that is neither module nor enum
    ///
    /// A struct or a trait qualifier imports an associated item, not
    /// variants, so the import keeps its diagnostic.
    #[test]
    fn import_of_another_item_is_flagged() {
        let diagnostics = run_with("fn f() { use Config::default; }", ImportSource::Other);
