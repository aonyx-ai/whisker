...
/// without re-querying the semantic model. A signature is attached to every
/// resolved function, even when nothing about it could be established: a
/// missing decoration reads identically to a function the provider never
/// reached, and those are different facts.
...
impl FnSignature {
    ...
    /// at all — including an `async fn` whose awaited type is not one, and
    /// one whose future could not be projected.
