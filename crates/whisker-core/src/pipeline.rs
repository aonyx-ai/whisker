...
impl Pipeline {
    ...
    /// provider's toolchain malfunctions, or if no provider claims the
    /// file — see [`Pipeline::run_on_source`].
    pub fn run(
        &mut self,
        path: &Path,
    ...
    ///
    /// Every provider is offered the file. If none claims it, the pipeline
    /// returns an [`UncoveredFile`] error rather than walking a tree with
    /// no semantic information, which would report the file clean while
    /// the semantic rules silently did nothing.
    ///
    /// One claim is enough. Two providers loaded for two workspaces can
    /// never both claim the same file, so demanding that all of them do
    /// would make the multi-workspace case impossible.
    ///
    /// Providers are merged in the order they are given, and the first
    /// decoration of a type on a node is the one lint rules see.
    ///
    /// # Errors
    ///
    /// Returns an error if parsing fails, if a provider's toolchain
    /// malfunctions, or if no provider claims the file — including the
    /// case where no providers were configured at all.
...
mod tests {
    ...
    ///
    /// Routing the assertion through a lint pass keeps the test honest: it
    /// observes what a real rule would see, not what the pipeline happened
    /// to store.
    struct ReportMarker;
